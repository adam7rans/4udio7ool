...
export default function Dashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'audio' | 'clips'>('audio');
    const [settings, setSettings] = useState({
        convert: true, // Deprecated/Always true effectively now that we have format
        format: 'mp3',
        segment: false,
        segmentDuration: 10,
    ...
    const handleGeneratePreview = async () => {
        ...
        const firstSelectedName = Array.from(selectedFiles)[0];
        const fileName = firstSelectedName;
    ...
    }, [settings.bitrate, settings.targetSize, settings.compressionMode, settings.format]);
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 space-y-8">
            ...
            <div className="flex justify-center gap-2 mb-6 bg-zinc-900/50 p-1 rounded-lg w-fit mx-auto border border-zinc-800">
                <button
                ...
                <button
                    ...
                    className={cn(
                        "px-4 py-1.5 rounded-md text-sm font-medium transition-all",
                        activeTab === 'clips' ? "bg-zinc-800 text-white shadow-sm" : "text-zinc-400 hover:text-zinc-200"
                    )}
                >
                    Clip DL
                </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6 space-y-6">
                            ...
                            <div className="space-y-3 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/50">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-zinc-500">Select audio format</p>
                                    </div>
                                </div>
                                <div className="grid grid-cols-4 gap-2">
                                    {['mp3', 'wav', 'aiff', 'aac'].map(format => (
                                        <button
                                            key={format}
                                            onClick={() => setSettings({ ...settings, format })}
                                            className={cn(
                                                "px-2 py-2 text-xs font-medium rounded-lg border transition-all uppercase",
                            ...
                            {/* Compression Settings (Bitrate or Target Size) - Only for compressed formats */}
                            {['mp3', 'aac'].includes(settings.format) && (
                                <div className="space-y-3 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/50 animate-in fade-in slide-in-from-top-1">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                            ...
                            {(settings.format === 'wav' || settings.format === 'aiff') && (
                                <div className="p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/50 animate-in fade-in slide-in-from-top-1 text-center">
                                    <p className="text-sm text-zinc-400">Lossless format selected.</p>
                                    <p className="text-xs text-zinc-600">Compression settings not applicable.</p>
                                </div>
                            )}
                        </div>

                        {/* Audio Preview Section - Show if any files selected */}
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-zinc-500">
                                            {previewUrl ? '15s preview ready' : `Hear quality on ${Array.from(selectedFiles)[0]} before batch`}
                                        </p>
                                    </div>
                                    <div className="flex gap-2">
                                        {!previewUrl ? (
                        ...
                        >
                            {isProcessing ? 'Processing Queue...' : `Batch Convert (${selectedFiles.size})`}
                        </button>
